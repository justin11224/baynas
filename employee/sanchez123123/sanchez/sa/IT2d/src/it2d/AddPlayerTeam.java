...
public class AddPlayerTeam {
    ...
    public void viewPlayerTeam() {
        ...
        while (true) {
            ...
            System.out.println("2. View team name and all players");
            ...
            switch (action) {
                case "1":
                    String query1 = "SELECT AddPlayerTeam.pt_id, Player.f_name, Player.l_name, Player.p, t.team_name, AddPlayerTeam.jd, AddPlayerTeam.r, AddPlayerTeam.s " +
                            "FROM AddPlayerTeam " +
                            "INNER JOIN Player ON AddPlayerTeam.p_id = Player.p_id " +
                            "INNER JOIN tbl_team t ON AddPlayerTeam.t_id = t.t_id";
                ...
                case "2":
                    String query4 = "SELECT t.team_name, p.f_name, p.l_name, p.p " +
                            "FROM tbl_team t " +
                            "INNER JOIN AddPlayerTeam apt ON t.t_id = apt.t_id " +
                            "INNER JOIN Player p ON apt.p_id = p.p_id " +
                            "ORDER BY t.team_name ASC, p.l_name ASC";
                    String[] headers4 = {"Team Name", "First Name", "Last Name", "Position"};
                    String[] columns4 = {"team_name", "f_name", "l_name", "p"};
